...
static int print_packet_line(FILE *fp, const packet_info_t *packet, const char *timestamp)
{
...
void logger_log_status(logger_t *logger, const char *level, const char *message)
{
    ...
    pthread_mutex_lock(&logger->lock);
    {
        int written = fprintf(logger->system_file, "[%s] [%s] %s\n", timestamp, level, message);
        if (written > 0) {
            logger->system_bytes_written += (uint64_t)written;
        }
    }
...
int logger_log_packet(logger_t *logger, const packet_info_t *packet)
{
    ...
    pthread_mutex_lock(&logger->lock);
    {
        int written = print_packet_line(logger->system_file, packet, timestamp);
        if (written > 0) {
            logger->system_bytes_written += (uint64_t)written;
        }
    }
...
int logger_log_alert(logger_t *logger, const alert_t *alert)
{
    ...
    pthread_mutex_lock(&logger->lock);
    {
        int written = fprintf(logger->alerts_text_file,
                              "[%s] [%s] [%s] src=%s dst=%s risk=%d confidence=%d cid=%s reason=\"%s\"\n",
                              text_timestamp,
                              ...
                              alert->reason);
        if (written > 0) {
            logger->alert_bytes_written += (uint64_t)written;
        }
    }
    ...
    if (logger->alerts_json_file != NULL) {
        ...
        fputs(",\"confidence_score\":", logger->alerts_json_file);
        fprintf(logger->alerts_json_file, "%d", alert->confidence_score);
        ...
        /* Approximate byte count: avoids per-write tracking of each fputs;
         * rotation is triggered conservatively on the next write cycle. */
        logger->alert_json_bytes_written +=
            64U /* static keys */ +
            strlen(json_timestamp) +
            strlen(ids_severity_name(alert->severity)) +
            strlen(detection_alert_type_name(alert->type)) +
            strlen(alert->source_ip) +
            strlen(alert->destination_ip) +
            strlen(alert->reason) +
            64U; /* numeric fields + formatting */
    }
    ...
    if (!logger->quiet) {
        printf("[%s] [%s] [%s] src=%s dst=%s risk=%d confidence=%d reason=\"%s\"\n",
               text_timestamp,
               ids_severity_name(alert->severity),
               detection_alert_type_name(alert->type),
               alert->source_ip,
                      ...
                      size_t data_len)
{
    ...
    if (alert_count > 0 && logger->pcap_file != NULL) {
        ...
        pthread_mutex_lock(&logger->lock);
        for (context_index = 0; context_index < logger->context_count; context_index++) {
            size_t index = (logger->context_index + logger->context_limit - logger->context_count + context_index) % logger->context_limit;
