...
void config_set_defaults(app_config_t *config)
{
    ...
    ids_copy_string(config->sqlite_path, sizeof(config->sqlite_path), "data/specterids.db");
    ids_copy_string(config->plugin_dir, sizeof(config->plugin_dir), "plugins");
    ids_copy_string(config->bpf_filter, sizeof(config->bpf_filter), "ip or ip6 or arp");
    ...
    config->beaconing_min_hits = 8;
    config->beaconing_interval = 30;
    config->beaconing_tolerance = 3;
    config->correlation_window = 300;
    config->pcap_speed = 1.0;
    config->json_logs = false;
    config->pcap_replay = false;
    config->benchmark_mode = false;
    config->pcap_export = false;
    config->metrics_enabled = false;
    config->sqlite_enabled = false;
    config->plugins_enabled = false;
    config->beaconing_ignore_private = true;
...
static void append_list_option(char *dst,
                               size_t dst_size,
                               const char *value,
                               const char *path,
                               unsigned long line_number)
{
    ...
    if (dst == NULL || value == NULL || dst_size == 0 || value[0] == '\0') {
        warn_config_line(path, line_number, "empty value ignored");
        return;
    }
    ...
    if (value_len >= dst_size || used + value_len + (used > 0 ? 2U : 1U) > dst_size) {
        warn_config_line(path, line_number, "list value is too long and was ignored");
        return;
    }
...
static bool parse_double_range(const char *value, double min, double max, double *out)
{
    char copy[32];
    char *endptr = NULL;
    double parsed;
    ...
    if (errno != 0 || endptr == copy || ids_trim(endptr)[0] != '\0') {
        return false;
    }
    if (parsed < min || parsed > max) {
        return false;
...
static void set_double_option(double *dst,
                              const char *value,
                              double min,
                              double max,
                              const char *path,
                              ...
                              const char *name)
{
    ...
    if (!parse_double_range(value, min, max, &parsed)) {
        char warning[128];
        snprintf(warning, sizeof(warning), "invalid range for %s ignored", name);
        warn_config_line(path, line_number, warning);
        return;
...
int config_load_file(app_config_t *config, const char *path)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        *separator = '\0';
        ...
        } else if (strcmp(key, "pcap_file") == 0 || strcmp(key, "pcap") == 0) {
            set_string_option(config->pcap_file, sizeof(config->pcap_file), value, path, line_number);
            config->offline_mode = config->pcap_file[0] != '\0';
        } else if (strcmp(key, "sqlite_path") == 0) {
            set_string_option(config->sqlite_path, sizeof(config->sqlite_path), value, path, line_number);
        } else if (strcmp(key, "plugin_dir") == 0) {
            set_string_option(config->plugin_dir, sizeof(config->plugin_dir), value, path, line_number);
        } else if (strcmp(key, "plugin") == 0) {
            append_list_option(config->plugin_paths, sizeof(config->plugin_paths), value, path, line_number);
            config->plugins_enabled = true;
        } else if (strcmp(key, "plugins") == 0 || strcmp(key, "plugin_paths") == 0) {
            set_string_option(config->plugin_paths, sizeof(config->plugin_paths), value, path, line_number);
            config->plugins_enabled = config->plugin_paths[0] != '\0';
        ...
        } else if (strcmp(key, "beaconing_whitelist") == 0) {
            set_string_option(config->beaconing_whitelist, sizeof(config->beaconing_whitelist), value, path, line_number);
        ...
        } else if (strcmp(key, "detection_shards") == 0) {
            set_int_option(&config->detection_shards, value, 1, 256, path, line_number, "detection_shards");
        ...
        } else if (strcmp(key, "beaconing_min_hits") == 0) {
            set_int_option(&config->beaconing_min_hits, value, 2, 100000, path, line_number, "beaconing_min_hits");
        } else if (strcmp(key, "beaconing_interval") == 0) {
            set_int_option(&config->beaconing_interval, value, 1, 86400, path, line_number, "beaconing_interval");
        } else if (strcmp(key, "beaconing_tolerance") == 0) {
            set_int_option(&config->beaconing_tolerance, value, 0, 86400, path, line_number, "beaconing_tolerance");
        } else if (strcmp(key, "correlation_window") == 0) {
            set_int_option(&config->correlation_window, value, 10, 86400, path, line_number, "correlation_window");
        } else if (strcmp(key, "pcap_speed") == 0) {
            set_double_option(&config->pcap_speed, value, 0.1, 100.0, path, line_number, "pcap_speed");
        } else if (strcmp(key, "json_logs") == 0) {
            set_bool_option(&config->json_logs, value, path, line_number, "json_logs");
        } else if (strcmp(key, "pcap_replay") == 0) {
            set_bool_option(&config->pcap_replay, value, path, line_number, "pcap_replay");
        } else if (strcmp(key, "benchmark_mode") == 0) {
            set_bool_option(&config->benchmark_mode, value, path, line_number, "benchmark_mode");
        ...
        } else if (strcmp(key, "sqlite_enabled") == 0) {
            set_bool_option(&config->sqlite_enabled, value, path, line_number, "sqlite_enabled");
        } else if (strcmp(key, "plugins_enabled") == 0) {
            set_bool_option(&config->plugins_enabled, value, path, line_number, "plugins_enabled");
        } else if (strcmp(key, "beaconing_ignore_private") == 0) {
            set_bool_option(&config->beaconing_ignore_private, value, path, line_number, "beaconing_ignore_private");
        } else if (strcmp(key, "no_color") == 0) {
            set_bool_option(&config->no_color, value, path, line_number, "no_color");
...
void config_print_effective(const app_config_t *config)
{
    ...
    printf("  input_mode: %s\n", config->offline_mode ? "OFFLINE" : "LIVE");
    printf("  pcap_file: %s\n", config->pcap_file[0] != '\0' ? config->pcap_file : "(not set)");
    printf("  pcap_replay: %s\n", config->pcap_replay ? "true" : "false");
    printf("  pcap_speed: %.2fx\n", config->pcap_speed);
    printf("  benchmark_mode: %s\n", config->benchmark_mode ? "true" : "false");
    printf("  log_dir: %s\n", config->log_dir);
    printf("  rules_file: %s\n", config->rules_file);
    ...
    printf("  sqlite_enabled: %s\n", config->sqlite_enabled ? "true" : "false");
    printf("  sqlite_path: %s\n", config->sqlite_path);
    printf("  plugins_enabled: %s\n", config->plugins_enabled ? "true" : "false");
    printf("  plugin_dir: %s\n", config->plugin_dir);
    printf("  plugin_paths: %s\n", config->plugin_paths[0] != '\0' ? config->plugin_paths : "(none)");
    printf("  no_color: %s\n", config->no_color ? "true" : "false");
    printf("  correlation_window: %ds\n", config->correlation_window);
}
