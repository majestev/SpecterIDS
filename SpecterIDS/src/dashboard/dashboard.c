                    ...
                    int refresh_ms)
{
    ...
    dashboard->color_enabled = color_enabled;
    ...
    if (mode != NULL &&
        ...
        ids_copy_string(dashboard->mode, sizeof(dashboard->mode), mode);
    } else {
        ids_copy_string(dashboard->mode, sizeof(dashboard->mode), "detailed");
    }
...
static void dashboard_render_locked(dashboard_t *dashboard, bool final_summary)
{
    ...
    if (!final_summary && dashboard->live_enabled && dashboard->color_enabled) {
        printf("\033[2J\033[H");
    }
...
static const char *severity_color(ids_severity_t severity, bool color_enabled)
{
    if (!color_enabled) {
        return "";
    }
...
void dashboard_render_stats(dashboard_t *dashboard, ids_stats_t *stats, bool final_summary)
{
    ...
    if (!final_summary && dashboard->live_enabled && !dashboard->quiet && dashboard->color_enabled) {
        printf("\033[2J\033[H");
    }

    printf("%sSpecterIDS %s%s | iface=%s | mode=%s | uptime=%.1fs\n",
           dashboard->color_enabled ? "\033[1m" : "",
           final_summary ? "summary" : "dashboard",
           dashboard->color_enabled ? "\033[0m" : "",
           dashboard->interface_name[0] != '\0' ? dashboard->interface_name : "(unknown)",
           dashboard->mode,
           snapshot.uptime_seconds);
    printf("  packets=%" PRIu64 " parsed=%" PRIu64 " drops=%" PRIu64 " parse_errors=%" PRIu64
           " dropped_alerts=%" PRIu64 " pps=%.2f mbps=%.3f alerts/min=%.2f\n",
           snapshot.captured_packets,
           snapshot.parsed_packets,
           snapshot.dropped_packets + snapshot.queue_drops,
           snapshot.parse_errors,
           snapshot.dropped_alerts,
    ...
    printf("  queues packet=%zu parsed=%zu log=%zu | cpu=%.3fs mem=%ldKB\n",
           ...
           snapshot.memory_kb);
    printf("  latency avg parse=%.3fus detection=%.3fus correlation=%.3fus logging=%.3fus | queue_pressure=%.0f\n",
           snapshot.avg_parse_us,
           snapshot.avg_detection_us,
           snapshot.avg_correlation_us,
           snapshot.avg_logging_us,
           snapshot.queue_pressure);
    printf("  alerts LOW=%s%" PRIu64 "%s MEDIUM=%s%" PRIu64 "%s HIGH=%s%" PRIu64
           "%s CRITICAL=%s%" PRIu64 "%s total=%" PRIu64 "\n",
           severity_color(IDS_SEVERITY_LOW, dashboard->color_enabled),
           snapshot.alerts_by_severity[IDS_SEVERITY_LOW],
           dashboard->color_enabled ? "\033[0m" : "",
           severity_color(IDS_SEVERITY_MEDIUM, dashboard->color_enabled),
           snapshot.alerts_by_severity[IDS_SEVERITY_MEDIUM],
           dashboard->color_enabled ? "\033[0m" : "",
           severity_color(IDS_SEVERITY_HIGH, dashboard->color_enabled),
           snapshot.alerts_by_severity[IDS_SEVERITY_HIGH],
           dashboard->color_enabled ? "\033[0m" : "",
           severity_color(IDS_SEVERITY_CRITICAL, dashboard->color_enabled),
           snapshot.alerts_by_severity[IDS_SEVERITY_CRITICAL],
           dashboard->color_enabled ? "\033[0m" : "",
           snapshot.alert_count);

    if (!compact) {
        printf("  protocols UNKNOWN=%" PRIu64 " TCP=%" PRIu64 " UDP=%" PRIu64
               " ICMP=%" PRIu64 " ICMPv6=%" PRIu64 " ARP=%" PRIu64 "\n",
               ...
               snapshot.protocols[PACKET_PROTO_ICMPV6],
               snapshot.protocols[PACKET_PROTO_ARP]);
        printf("  ip-family IPv4=%" PRIu64 " IPv6=%" PRIu64 " ARP=%" PRIu64
               " malformed=%" PRIu64 " shards=%" PRIu64 "\n",
               snapshot.ipv4_packets,
               snapshot.ipv6_packets,
               snapshot.arp_packets,
               snapshot.malformed_packets,
               snapshot.detection_shards);
        if (strcmp(dashboard->mode, "advanced") == 0) {
            printf("  runtime shard_pressure=%.3f plugins=%" PRIu64
                   " plugin_packets=%" PRIu64 " plugin_alerts=%" PRIu64 "\n",
                   snapshot.shard_pressure,
                   snapshot.plugin_count,
                   snapshot.plugin_packets,
                   snapshot.plugin_alerts);
            printf("  storage writes=%" PRIu64 " errors=%" PRIu64
                   " | events published=%" PRIu64 " dispatched=%" PRIu64
                   " dropped=%" PRIu64 " queue=%zu/%zu\n",
                   snapshot.storage_writes,
                   snapshot.storage_errors,
                   snapshot.event_published,
                   snapshot.event_dispatched,
                   snapshot.event_dropped,
                   snapshot.event_queue_depth,
                   snapshot.event_queue_capacity);
        }
