...
static packet_info_t make_ipv6_tcp_packet(const char *src, const char *dst, uint16_t dst_port, time_t seconds)
{
    packet_info_t packet = make_tcp_packet(src, dst_port, seconds);

    snprintf(packet.dst_ip, sizeof(packet.dst_ip), "%s", dst);
    packet.ip_version = 6;
    return packet;
}
...
static void test_sharded_ipv6_detection(void)
{
    ids_rules_t rules;
    detection_engine_t *engine;
    alert_t alerts[4];
    ...
    rules_set_defaults(&rules);
    rules.port_scan.threshold = 2;
    rules.ssh_bruteforce.enabled = false;
    rules.syn_flood.enabled = false;
    rules.beaconing.enabled = false;

    engine = detection_create_with_shards(&rules, 8);
    assert(engine != NULL);

    for (port = 80; port < 83; port++) {
        packet_info_t packet = make_ipv6_tcp_packet("2001:db8::20", "2001:db8::10", (uint16_t)port, 5000 + port - 80);
        count = detection_process_packet(engine, &packet, alerts, 4);
    }
